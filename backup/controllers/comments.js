...
module.exports = function (mongoose, utils, constants) {
  ...
  commentsCtrl.viewNotification = async (req, res) => {
    try {
      ...
      askedQuestions = askedQuestions.map((e) => {
        return {
          ...e,
          ago: moment(e.createdAt).fromNow(),
          shortName: e.user_profiles.fullName
            .split(" ")
            .map((word) => word.charAt(0))
            .join(""),
        };
      });
      answeredQuestions = answeredQuestions.map((e) => {
        return {
          ...e,
          ago: moment(e.createdAt).fromNow(),
          replyAgo: moment(e.replies.createdAt).fromNow(),
          shortName: e.user_profiles.fullName
            .split(" ")
            .map((word) => word.charAt(0))
            .join(""),
